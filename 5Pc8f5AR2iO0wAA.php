...
$body = file_get_contents('php://input'); //Получаем в $body json строку от Jira
...
if (isset($arr['webhookEvent'])) { // Проверяем есть ли какое-то обновление
    ...
    if ($data['webhookEvent']=='jira:issue_created') { // Если поступила информация о том, что была создана новая задача
        ...
        exit('Ok'); // Завершаю работу, сообщив в Jira, что данные были корректно получены (сохранил инфу по созданной задаче)
    }
    if ($data['webhookEvent']=='jira:issue_updated') { // Если поступила информация о том, что был обновлен запрос
        ...
        exit('Ok'); // Завершаю работу, сообщив Jira, что данные были корректно получены (сохранил данные по обновлению запроса)
    }
    exit('Ok'); // Выходим, это не создание задачи и не обновление статуса задачи (нечего делать)
}
else (exit('Отсутствует webhookEvent')); // Завершаю работу, сообщив в Jira, что данные не были получены (даже webhookEvent не увидел)
